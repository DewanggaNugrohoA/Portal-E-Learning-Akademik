...
<div class="page">
    <div class="container-small">
        <div class="header">
            ...
            <p>Data mata pelajaran akan diperbarui menggunakan API endpoint /api/mata-pelajaran/{{ $id }}.</p>
        </div>
            ...
            <form id="editMataPelajaranForm">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="kode_mata_pelajaran" id="kode_mata_pelajaran" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="guru_pengampu" id="guru_pengampu" required>
                    </div>

                    <div class="form-group">
                        <label>Jam Pelajaran</label>
                        <input type="number" name="jam_pelajaran" id="jam_pelajaran" required>
                    </div>
...
<script>
    ...
    async function loadDetailMataPelajaran() {
        try {
            ...
            if (!response.ok) {
                showPopup('Gagal', result.message ?? 'Data mata pelajaran tidak ditemukan.', 'error', '/mata-pelajaran');
                return;
            }

            const mapel = result.data;

            setInputValue('kode_mata_pelajaran', mapel.kode_mata_pelajaran);
            setInputValue('nama_mata_pelajaran', mapel.nama_mata_pelajaran);
            setInputValue('guru_pengampu', mapel.guru_pengampu);
            setInputValue('jam_pelajaran', mapel.jam_pelajaran);
            setInputValue('semester', mapel.semester);
            setInputValue('status', mapel.status ?? 'Aktif');
        } catch (error) {
            showPopup('Gagal', 'Gagal memuat data mata pelajaran.', 'error', '/mata-pelajaran');
        }
    }

    document.getElementById('editMataPelajaranForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            ...
            if (!response.ok) {
                let message = 'Gagal memperbarui data mata pelajaran.';
            ...
            showPopup('Berhasil', result.message ?? 'Data mata pelajaran berhasil diperbarui.', 'success', '/mata-pelajaran');
        } catch (error) {
            showPopup('Gagal', 'Terjadi kesalahan saat memperbarui data mata pelajaran.', 'error');
        }
    });
